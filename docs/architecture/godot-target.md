...
The test fixture saves/reopens a serialized scene through editor APIs, checks token fallback values in loaded Theme resources, executes graphics-backed observations and exports a fresh macOS application. The comparison runs Unity Editor-panel traces against Godot runtime/export traces. It requires matching run IDs and exact expected sequences. It does not claim input-device integration, accessibility, arbitrary composition, full visual parity or shared traces executed in a Unity Player.

The foreground Theme projection is intentionally narrow; other styling remains unsupported and explicit. The changed-token probe reads `Theme.get_color()` and frees the probe before rendering. It proves that the changed value reached the loaded Theme resource, not that a rendered control consumed it or that pixels changed. The normal-control screenshot is a separate rendering witness. Expanding component admission should follow enforced capability decisions and independent consumer workflows, not the number of scenes that compile.
