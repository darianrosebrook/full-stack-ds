...
18. **A coordinate is emitted only where the declaration carries the fact that distinguishes it from its encoding.** The necessity census derives its propositions from the emitted declaration's SHAPE, and shape alone does not say whether a sequence's order is semantic, whether a required child's presence is independent, or which branch a payload belongs to. Two classes measure the cost. The `#present` cross-terms on required children: thirteen discriminator member-absences that were three holder-presence facts, fixed by reading `optional.self`/`optional.holder` instead of ancestry. And `#order` on set-valued name lists, where the declaration is a JSON array standing in for a set: measured by permuting each list at its own slot, `project.keep` and `structure.peers[]` never move a judgment over any committed instance, `nest.levels` does, and the five plain name-list declarations are shape-identical (`{minItems, type: array, items: {$ref: name}}`) so nothing in the emitted schema can tell the census which is which. The requirement is on the DECLARATION — set versus sequence, optionality, branch identity — not on the walk's willingness to guess; where the fact is absent the coordinate is a proposition nothing can adjudicate, and it is not repaired by adding corpus the coordinate's own erasure cannot reach.
...
- **A reference the derivation boundary must RESOLVE has no quotient that forgets its co-reference.** The census carries an `#incidence` coordinate on every name reference, and its erasure rewrites the names at that slot to reserved tokens, so sharing through the slot becomes unobservable while every other reference keeps its identity. For a reference inside `assertions[]`, or one the boundary does not read, that image is a legal declaration and the erasure is answerable. For a **derivation operand** it is not: forgetting which declared names `nest.levels`, `join.with` or `graph.edgeFrom` share leaves the operator with an input it cannot resolve, and the boundary reports `REL_DERIVATION_INPUT_MISSING` — or `REL_DERIVATION_RESULT_NOT_DERIVABLE` where the forgotten names were the result's own fields. Those are the boundary's well-formedness refusals, which stay collateral for every class by design, because a collision found under one may be the break rather than the coordinate. So `checkIsolation` refuses the erasure on every stimulus it touches and no witness naming the coordinate can be admitted, however much corpus authority is added. Measured over the 39 live `reference-topology` coordinates, **18 have an erasure that is never discharged on any fixture where it moves bytes** — sixteen `relation.derivedBy.*` operand references (fifteen `#incidence` plus `project.keep#arity`), `structure.peers[]#incidence` — which is 18 of that basis's 33 unadjudicated `reference-topology` facets, the other 15 being nine that erase nothing anywhere and six that are discharged and merely await a separating pair. **The distinction such a coordinate names is nonetheless real, and it is measured rather than conceded.** On `FX_N_NESTED_SUBTOTAL_AT_PREFIX`, rebinding the first level from `country` to `revenue` moves exactly one path, leaves both sides with a clean derivation boundary, and flips the verdict from admissible to `REL_GRAIN_SUBTOTAL_MISMATCH` under the already-existing cause `CASE_NESTED_SUBTOTALS_OFF_GRAIN`. Erasing the coordinate DOES identify those two stimuli, so the collision obligation HOLDS and isolation alone fails — which is why the committed `nest.levels#order` witness over the same pair is accepted while that pair re-cited for `#incidence` is refused, and why an authored `{base, patch}` stimulus cannot mend it. The absence of a verdict here is a **limit of the quotient, not missing corpus coverage**, and it is not a disposition this vocabulary can name: not an artifact (the erasure does identify two separated representations), not derived vocabulary (the degree of freedom is independent), not not-yet-admitted (active authority requires the distinction). The question it opens is the quotient itself — an erasure that forgets which declared names a bound reference shares without leaving it unresolvable — and the shape that distinction would take is already declared and unemitted: `Coordinate.slots`, "the sibling reference slots whose assignment is at stake", is a `binding` facet the walk never produces. `C1f` in `necessity.test.ts` pins the partition, the refusal's cause, the oracle-separated pair, and the contrast with `#order`.
- **The ORDER facet is emitted for every name list, and the declaration cannot say whether the sequence is semantic.** `addReference` emits `arity`, `order` and `incidence` for a list slot and only `incidence` for a single name, with `list` read from the emitted schema. That schema carries no set-versus-sequence fact: the five plain name-list declarations — `keep`, `along`, `nonAdditiveAlong`, `levels`, `toGrain` — are identical but for `minItems`, while the authority lives in the laws (`project.keep` is compared with `sameSet(fieldNames(out), d.keep)`; `nest.levels` is compared positionally by `isDeclaredNestGrain`, and `toGrain` inherits that comparison). Measured by PERMUTING each list at its own slot — a different value, not an erasure — and comparing the judgment: `project.keep` and `structure.peers[]` never move it over the four instances each has that are long enough to permute, so their `#order` measures the encoding and no witness for either can exist however much corpus is added; `nest.levels` moves it in two of its five, so it is a real degree of freedom and already primitive; and `along`, `nonAdditiveAlong`, `grainWitness` and `toGrain` have no committed instance long enough to permute, so the corpus cannot decide them. **`toGrain#order` is nonetheless semantic**, and carries a holding witness built from an authored pair whose two sides differ only at the target grain — which is exactly why the untested class must never be read as inert. A further asymmetry runs the same way: `relation.grain` is a name list declared as a union, so the walk emits the leaf and none of the three facets at all, and its order is inert over the 32 multi-element instances the corpus carries. What is missing is again a fact on the DECLARATION rather than a defect in the walk, and `C1g` pins the partition, the shape-identity that explains it, the `grain` asymmetry, and the authored pair that reaches what the corpus cannot.
