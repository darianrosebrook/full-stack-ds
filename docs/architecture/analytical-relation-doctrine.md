...
- **The erasure instrument cannot witness a coordinate whose ABSENCE is itself diagnosed.** A witness needs an erasure that leaves the surrounding representation well-formed: two stimuli must be distinguishable before it and identical after, with the erased side still a legal declaration. Where the engine diagnoses the *absence* of a coordinate, that condition cannot be met — the erasure is the violation, so the erased side is not a stimulus but the defect. Measured on `relation.derivedBy.bin.closure`: the field is optional in the schema, so `delete-slot` is the correct erasure and the erased declaration is schema-valid, yet deleting it introduces `REL_BIN_CLOSURE_UNDECLARED` — the diagnostic of `CASE_WHICH_BIN_GETS_TEN`, whose `asserted` is literally *bin with no declared closure* and whose terms carry `invariant:declared-closure`. The operation is not what decides this: `assertion.aggregate.nulls:exclude~<absent>` uses the same `spell-member-as-absent`, and IS witnessable, because an absent null policy is a legal default the engine does not diagnose. Such a coordinate is neither an artifact nor derived vocabulary, and its necessity is stated by the diagnostic that names its absence rather than by a discriminating pair. **The disposition vocabulary has no verdict for that shape.** `witnessed` requires a holding witness; `representation-artifact` and `not-yet-admitted` both mean the coordinate leaves the kernel, which would delete a distinction whose absence the corpus diagnoses; `required-derived-vocabulary` says the name is external authority's rather than the engine's own. Which verdict a *declared* necessity takes is open, and until it is decided those coordinates stay `unresolved` rather than being filed under a verdict that would remove them. **And the pair it refuses is the ideal one.** Sweeping every unresolved stage-2 candidate for a pair the erasure is a no-op on one side of — the shape that makes a collision evidence about PRESENCE rather than about whatever the erasure destroys with it — finds exactly two, and they are the two `bin.closure` spellings: one fixture is literally the other minus the declaration, illegal against admissible. The corpus supplies no third. So the refusal is not the absence of evidence; it is `checkWitness`'s isolation clause declining a pair whose only effect is to produce the configuration the corpus diagnoses, which is the witness's content rather than a substitution that exchanged something else. Whether that clause should distinguish those two cases is the open question, and it is the same one as the missing verdict seen from the instrument's side. By contrast `evidence.rows.*#present` has no such pair at all: 161 of 210 specimens carry no `evidence.rows`, so presence is varied widely, but no fixture IS another fixture plus rows, which is what collision requires once the erasure leaves a row-less fixture untouched.
