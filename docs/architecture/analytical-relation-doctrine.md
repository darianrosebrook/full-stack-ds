...
17. **A distinction is counted once.** An enum leaf and its member pairs describe one value space: for a `k`-member enum the census emits the leaf plus C(k,2) member pairs against a distinction space of rank k−1. **The measurement over all eleven enum leaves settles the direction, and it is stronger than the k=2 case that exposed it:** `footprint(leaf)` is exactly `{leaf}` together with every one of its member pairs, and each pair's footprint is only itself — so erasing the leaf destroys everything erasing any pair destroys, while no pair subsumes the leaf. The leaf is the COARSEST coordinate of its enum and its necessity is IMPLIED by any single member pair; it carries no independent degree of freedom, only the name for *this value is determined at all*. **The decided factoring is that the member pairs are the primitive distinctions and the bare leaf is `required-derived-vocabulary`** — which is what the subtraction policy's `tagIsNotTheChoice` and `reportingVocabulary` probes already describe, and the answer to the `kindInterpretation.openQuestion` this document left open for `relation.derivedBy.kind`. At k=2 the leaf and its sole pair additionally coincide at the specimen level, which the footprint rules correctly decline to claim, because claiming it would make them read enum arity while the census is the single reader of the schema. **Implementing this cannot start from the witness.** Re-filing the misattributed `field.temporality.kind` witness leaves the leaf unratified, and the evidence machinery's three guarantees — a suspension points at a current failure, a suspension re-derives when acceptance moves, and every historical loss is ledgered — are then jointly unsatisfiable for that coordinate, because the failing witness IS the record of its loss: measured, not argued, and the consumer that refuses it is right to. **Implementing it needs one more accounting mode before it can start anywhere.** `required-derived-vocabulary` is the third accounting mode — `experiments.ts` names it, and the subtraction's `dispositions` glossary defines it — but `CurrentSupport` carries only the three evidence classes, so a retained coordinate reads as unsupported and its loss is filed as unexplained. A basis that records the verdict holds every gate but two, and both failures are the machinery stating the missing mode: coverage reports the codomain entry as no longer needed once a basis owns the leaf, while the historical comparison refuses to call the leaf accounted. The census is therefore not the first place to change — `CurrentSupport` is; until the mode exists the leaf keeps the witness it has. Prevents a normal form that counts one distinction twice and then demands a witness for each copy.
