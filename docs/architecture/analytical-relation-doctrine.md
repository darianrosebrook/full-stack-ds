...
governs:
  ...
  # Proposed; do not exist until stage 1 lands:
...
This document is a **draft architecture doc**: a proposed shape, ratified only in part. What exists is the **corpus contract** — `packages/ds-contracts/analytical-pack/` (vocabulary, form-name denylist, case schema, corpus) and `packages/ds-codegen/src/analytical/corpus-integrity.ts`, which binds the corpus and this document to each other mechanically (the diagnostic catalogue and the vocabulary appendix below are parsed by it). That is stage 0 and 0.5 of "Order of construction". Everything from the relation schema onward — the field algebra as executable types, the engines, the projection space, the realizations — is provisional and unbuilt. The doctrine's shape is ratified, and `status` flips to `active`, when stage 1 lands an engine that adjudicates every stage-1 corpus case three-valued (admissible / illegal / unproven) with the expected diagnostic or obligation, and carries a necessity witness for every axis it retains. Until then this doc confers no authority (see `document_governance.md`, "Drafts confer no authority").
...
| 1 | L0–L2 as executable types over a **relational structure**; the three checkability engines as pure functions; the **three-valued** judgment with evidence classes; a heterogeneous fixture (per-observation provenance, nulls, uncertainty); a **necessity witness** per retained axis; the pre-schema recon that expresses the four probes (a categorical-vs-ratio relation, a binned ratio field, an OHLC-over-interval relation, a nested hierarchy) plus a graph with an isolated node. | Every **stage-1** corpus case returns the expected judgment: illegal cases with their diagnostic, unproven cases with their obligation, and a legal corpus (new) admissible. The engine's distinct diagnostics over stage-1 cases equal the catalogue's stage-1 diagnostics. | Undercount (two catalogue causes collapse into one emitted diagnostic) or overcount (a diagnostic per case: overfitting); an axis with no discriminating pair; a probe expressible only with a form-specific field; the column-level qualifier design surviving the heterogeneous fixture. |
...
9. **Axis without necessity.** An L0–L2 axis survives into the relation schema with no discriminating pair — no two structures that differ only in that axis and differ in legality or derived meaning. (Stage 1.)
