...
17. **A distinction is counted once.** An enum leaf and its member pairs describe one value space: for a `k`-member enum the census emits C(k,2) member pairs against a distinction space of rank k−1, and at k=2 the sole member pair and the leaf partition the slot identically — merging the only two members leaves no third member to tell them apart, so no stimulus witnesses one without also witnessing the other. The two live instances are `field.temporality.kind` and `assertion.kind`, and each carries a leaf witness that no stimulus can repair without ratifying the same binary distinction twice. The footprint rules correctly decline to claim that coarsening, because claiming it would make them read enum arity and the census is the single reader of the schema; the factoring is therefore the census's to make, and it is not yet made. **The order is forced rather than chosen.** Re-filing the misattributed `field.temporality.kind` witness before the factoring is decided leaves the leaf unratified with no acceptance boundary able to re-derive it, and the evidence machinery's three guarantees — a suspension points at a current failure, a suspension re-derives when acceptance moves, and every historical loss is ledgered — are then jointly unsatisfiable for that single coordinate: measured, not argued, and the consumer that refuses it is right to. Decide the factoring and the surviving coordinate keeps a real witness, at which point the whole thread dissolves. Prevents a normal form that counts one distinction twice and then demands a witness for each copy.
