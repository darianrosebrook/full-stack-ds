...
| Runtime visual proof rail | A separate CI job runs after the main gate and exercises Progress, Truncate, ShowMore, OTP, and Calendar in real Chromium through React, Vue, Svelte, and Lit preview mounts. It asserts contract facts, not screenshot pixels. | implemented / CI-gated for fact assertions | `e2e/runtime-rail.spec.ts`; `.github/workflows/ci.yml`; commit `9c7bead`; CI fix `5f197b0` | Angular runtime preview is skipped; non-default props are not asserted; behavioral parity beyond DOM shape is not asserted. Screenshot baselines are darwin-only and skipped under `CI=true`. | Add a dedicated harness for non-default runtime props and decide whether Angular preview should become executable. |
...
| CSS custom-property bindings | Prop-to-CSS custom-property lowering exists and is runtime-checked for default/fallback cases in Progress, Truncate, and ShowMore. | implemented / runtime-proven for narrow defaults | `packages/ds-codegen/src/frameworks/css-var-bindings.test.ts`; `e2e/runtime-rail.spec.ts` | The rail does not prove every non-default computed visual geometry or every token cascade branch. | Add parameterized runtime routes that can mount explicit non-default props. |
...
| Non-web generation | Non-web remains a recon/prototype lane. SwiftUI artifacts and tests are useful pressure surfaces, but Web DOM is still the only executable multi-framework family. | experimental / not promoted | `docs/non-web-generation.md`; `packages/ds-codegen/src/frameworks/swift/**` | No claim of substrate-neutral component semantics, React Native readiness, or production native target admission. | Promote one hostile thin target through target-pack provenance and explicit residual extraction. |
...
The claim boundary is still bounded. It does not yet prove production adoption ergonomics, complete accessibility adequacy, visual quality, npm-distributed packages, live Figma library publication, executable external target packs, or substrate-neutral UI semantics.
