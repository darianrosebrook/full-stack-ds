...
updated: 2026-09-13
verified_at_commit: 7abe4d7b654531d22e6fe2238273916a582c2886
governs:
...
- Two registered native targets outside the admission rail consuming the same IR: SwiftUI, allowlisted for <!-- target-component-count:swiftui -->52 contracts — the full corpus — and Jetpack Compose, allowlisted for <!-- target-component-count:jetpack-compose -->31. Both generated roots are byte-drift-gated and compile through dedicated native CI lanes over hand-authored example consumers. SwiftUI additionally has CI-run body/state/resolver and sampled token-paint tests plus a separately invoked macOS press-proof harness for named interactions; Compose has resolver conformance tests but no UI-runtime lane. These are bounded facts, not broad native behavioral parity or device correctness.
...
The seventh and eighth targets extend that result at graded proof strength. SwiftUI (`frameworks/swift/`, outside the admission rail) now emits its allowlisted <!-- target-component-count:swiftui -->52 contracts — the full corpus — and Jetpack Compose (`frameworks/jetpack-compose/`, allowlisting <!-- target-component-count:jetpack-compose -->31) follows the same shape. Both consume the same substrate-aware IR facts, avoid per-component emitter-name lore in the enforced scope, are byte-drift-gated, and compile in dedicated native CI lanes. SwiftUI goes further: `swift test` evaluates every admitted component body and pins state, resolver, and sampled token-paint facts, while `PressProofHarness` separately exercises a bounded set of macOS interactions. Compose has no equivalent UI-runtime fact surface. None of that proves broad native behavioral or visual parity.
