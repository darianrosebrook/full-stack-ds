...
- <!-- component-count -->52 component contracts generated through one primitive. (Count is loader-derived — `contracts-fs.ts` walks `components/*/<Name>.contract.json`; do not hand-maintain this number, re-derive it.)
...
- Two registered native targets outside the admission rail consuming the same IR: SwiftUI, allowlisted for <!-- target-component-count:swiftui -->51 contracts — the full corpus — and Jetpack Compose, allowlisted for <!-- target-component-count:jetpack-compose -->23. Both generated roots are byte-drift-gated and compile through dedicated native CI lanes over hand-authored example consumers. SwiftUI additionally has CI-run body/state/resolver and sampled token-paint tests plus a separately invoked macOS press-proof harness for named interactions; Compose has resolver conformance tests but no UI-runtime lane. These are bounded facts, not broad native behavioral parity or device correctness.
...
This is the move that, applied consistently, produces the constraint of "one primitive, many components" in this codebase. Each time a surface request for a new primitive was refused, the contract grew the fields that made the new primitive unnecessary instead. The result is a corpus where the primitive count has stayed at one across <!-- component-count -->52 components; it is not a proof that the primitive count would stay at one indefinitely.
...
There is exactly one governed source corpus where the meaning of an artifact lives. In this repo, the semantic root is `packages/ds-contracts/components/<Name>/<Name>.contract.json`, validated against `component.contract.schema.json`; adjacent schema-validated token and style sidecars own its realization bindings. Usage sidecars are curated documentation inputs, not codegen semantics. Generated source, generated tests, CSS, and agent-facing descriptors derive from those authorities. A human changes the owning contract or sidecar, not the generated framework realization, to make a durable change.
...
The Web DOM component realization is composed around one rendered primitive: `Stack`, defined in `packages/ds-contracts/primitives/Stack.primitive.json`. There is no `Button` primitive, no `Input` primitive, no `Dialog` primitive. `Button` is `<Stack as="button">` with a contract that constrains its props. `Dialog` is `<Stack as="div" role="dialog">` plus compound parts, also stacks.

This is the constraint that pins the contract at the right level of abstraction. If a contract needs to escape into a new primitive to express something, the contract is missing a field. The intent is that every new behavior must be expressible *in the contract*, not in a new primitive. Across the current <!-- component-count -->52 components, no second primitive has been required; whether that continues under broader component pressure is open.
...
The seventh and eighth targets extend that result at graded proof strength. SwiftUI (`frameworks/swift/`, outside the admission rail) now emits its allowlisted <!-- target-component-count:swiftui -->51 contracts — the full corpus — and Jetpack Compose (`frameworks/jetpack-compose/`, allowlisting <!-- target-component-count:jetpack-compose -->23) follows the same shape. Both consume the same substrate-aware IR facts, avoid per-component emitter-name lore in the enforced scope, are byte-drift-gated, and compile in dedicated native CI lanes. SwiftUI goes further: `swift test` evaluates every admitted component body and pins state, resolver, and sampled token-paint facts, while `PressProofHarness` separately exercises a bounded set of macOS interactions. Compose has no equivalent UI-runtime fact surface. None of that proves broad native behavioral or visual parity.

The native work also found the boundary of the claim: the contract and IR own portable shape, channel, and static token facts; substrate interaction physics remain target-runtime responsibility unless the contract explicitly models them. That is not a framework escape hatch, but it is a narrower claim than "all UI semantics are substrate-neutral." For those native extensions, additional representation cost landed in the IR. The bounded Unity UI Toolkit pilot subsequently reused existing boolean-control, item-interaction and anchored-surface facts without extending the IR. Its shared C# runtime owns Unity event delivery and panel geometry; its initial visual skin and supported option adapters are bounded backend work, not proof of universal design portability.
...
It does not claim that the codebase is feature-complete or production-grade beyond its own scope. The <!-- component-count -->52 components were chosen to exercise the contract; they are not optimized for adoption.
...
1. **A system lacking one property that matches or beats this system on predeclared outcomes.** "Works well" cannot be defined by restating the seven properties. On a fixed corpus and target sequence, compare externally observable measures such as semantic-defect escape under seeded mutations, regeneration loss/divergence, marginal engineering time for the next heterogeneous target, and consumer breakage after renderer-internal refactors. Declare the workload and thresholds before observing the result. A system without a framework-neutral IR, fail-loud boundary linters, or consumer descriptors that matches those outcomes would falsify the necessity of property 2, 5, or 7 respectively.
