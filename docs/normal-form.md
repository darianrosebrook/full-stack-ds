...
- Two registered native targets outside the admission rail consuming the same IR: SwiftUI, allowlisted for <!-- target-component-count:swiftui -->52 contracts — the full corpus — and Jetpack Compose, allowlisted for <!-- target-component-count:jetpack-compose -->49. Both generated roots are byte-drift-gated and compile through dedicated native CI lanes over hand-authored example consumers. SwiftUI additionally has CI-run body/state/resolver and sampled token-paint tests plus a separately invoked macOS press-proof harness for named interactions; Compose has resolver conformance tests but no UI-runtime lane. These are bounded facts, not broad native behavioral parity or device correctness.
