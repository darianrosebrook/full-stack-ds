...
3. **Functional API** — a lane-local, typed, **promise-returning** surface the assembly calls: `listProducts()`, `quoteCart(cart)`, `validatePromo(code, cart)`, `submitPayment(payload)`, `placeOrder(order)`. Each resolves a typed `ApiResult<T>` (resolve-don't-reject for domain failures; see the portfolio README's "Golden data/API template"). The simulated load latency, the promo validity rule, and the mock payment-failure flag live **behind** this API, so the validating/pending/error states below are real async transitions, not synchronous fakes. The assembly never reads the fixture or adapter directly.

#### App-specific decision: totals/promo/shipping/tax policy lives behind the API

This is the checkout app's one app-specific data/API decision, pinned here so the framework lanes (and any subagent implementing the seam) do not diverge:

- **Totals, promo validation, the shipping/tax quote, payment simulation, and order placement all live behind the lane-local functional API** — not in each framework UI. `quoteCart(cart)` returns the full derived breakdown (subtotal → discount → shipping → tax → total) as already-computed numbers; `validatePromo(code, cart)` returns the validated promo + its effect on the quote; `submitPayment` and `placeOrder` simulate the commit.
- **The assembly (UI) owns cart/form/step state only.** It holds the line items, the form field values, and the explicit step machine (`contact | shipping | payment | review | confirmation`), and it *calls* the API for every derived money value.
- **Future framework lanes must not independently recompute totals** (subtotal/discount/shipping/tax/total) per framework. They consume the API-returned numbers and may only apply **display formatting** (currency symbol, locale, rounding presentation) to those values. Re-deriving the math in a lane is a parity finding: five hand-written totals calculators will drift, and the point of the shared API is that the policy is computed once.
- **This does not move product policy into FSDS packages.** The API is *lane-local app code*, not package code — exactly where the assembly methodology says product policy belongs (`src/views/ComponentComplexityView.tsx`: assemblies own product policy; the system provides building blocks). The boundary being protected is "FSDS package vs. app", not "UI vs. data layer"; keeping totals behind the lane's own API honors both.
...
- **Derived state:** the order summary (subtotal → discount → shipping → tax → total) is **quoted by the API** (`quoteCart`), not recomputed in the UI — the lane holds the returned numbers; the cart count badge, the per-step validity (derived from field validation), the aggregate "can advance" gate per step, and the "can checkout" gate (cart length > 0) are UI-derived from cart/form state. The split: money math is API-owned (one calculator); UI-shape gates are assembly-owned.
