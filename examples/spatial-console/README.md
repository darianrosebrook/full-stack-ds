...
5. **Package finding, fixed in the system (DS-CARD-HEADER-VUE-TESTID-01): the generated Vue components dropped a template-passed `data-testid`.** Vue stores a declared prop under its camelized key, so the template's `props['data-testid']` read was always undefined; every generated Vue component had it. The emitter now declares and reads `dataTestid`, and every generated Vue suite asserts that a passed `data-testid` lands in the rendered DOM. The verifier still selects the Vue switch by `role`.
6. **Package finding, fixed in the system (DS-CARD-HEADER-VUE-TESTID-01): `CardHeader` was exposed as a `banner` landmark.** Any part named `header` or `footer` rendered as `<header>`/`<footer>`, which outside a sectioning ancestor is the page's banner/contentinfo. That hit React, Vue and Svelte across Card, Calendar, Dialog, Field, Postcard and Sheet. Lit and Angular treat `as` as advisory and never rendered those elements. The parts now render as `div`; a contract that wants the element declares `anatomy.details.<part>.tag`.
...
8. **Package finding, fixed in the system (DS-CONSUMER-FINDINGS-01): `modal` on `Sheet` and `Dialog` was a no-op.** The contracts now declare `surface.modalityProp: "modal"`. With `modal={false}` the focus trap and scroll lock stay off, the backdrop is hidden, and `aria-modal` reads `"false"` in all five web frameworks (`e2e/modality-rail.spec.ts`). The same work found that the Lit focus trap never wrapped, because it compared `document.activeElement` (the shadow host) to the panel's last focusable.
9. **Package finding, fixed in the system (DS-CONSUMER-FINDINGS-01): `Toast` never rendered its action.** The action is now a named slot (`slots.action` in React, `<slot name="action">` in Vue and Lit, a snippet in Svelte, `[slot=action]` projection in Angular), and its wrapper renders only when the slot is filled (`if: "slot:action"`). Angular cannot know statically which projections are filled, so it always renders the wrapper; `.toast__action:empty { display: none }` hides it when unfilled (`e2e/toast-live-region-rail.spec.ts`).
10. **Package finding, fixed in the system (DS-CONSUMER-FINDINGS-01): the `Toast` viewport was `role="alert"`.** That role implies assertive and overrode `aria-live={politeness}`. The viewport is now a `role="region"` labelled "Notifications", `aria-live` follows `politeness`, and each item keeps `role="status"`. The viewport stays mounted while no toast is open, which is correct: a live region must exist before content is inserted into it. Not checked with a screen reader.
11. **Brand finding, addressed at the component (DS-CONSUMER-FINDINGS-01): primary and destructive buttons rendered the same red.** The selected brands alias primary to a red. The destructive `Button` is now outlined (no fill, danger border and label), so it stays distinct under any brand whose primary is red (`e2e/button-intent-rail.spec.ts`). The brands themselves are unchanged.
