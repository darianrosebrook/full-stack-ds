...
| `spatial-console/` | `web/` (React + Vue + Lit in one page) | experimental; locally runtime-verified, not in CI | React, Vue and Lit panels built from public package exports can be drawn into a WebGL scene via WICG HTML-in-Canvas and act as input surfaces for the scene through the browser's own hit testing. Requires a flagged Chrome; see [`spatial-console/README.md`](./spatial-console/README.md) for findings and non-claims. |
