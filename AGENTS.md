...
2. **Create a bound worktree — never a plain branch.** `caws worktree create <name> --spec <SPEC-ID> --base-branch main`, then do ALL work under `.caws/worktrees/<name>/`. The binding enforces `scope.in`; gitignored dependency artifacts (`node_modules`, `.pnpm-store`) are symlinked in — but the shared `node_modules` makes vitest resolve duplicate module instances (a second svelte copy reds the suite with `rune_outside_svelte`), so run `pnpm run worktree:install` once after create to replace the links with a real frozen install off the shared store (`pnpm run worktree:check` re-verifies isolation). Gitignored build outputs are not linked at all — rebuild `packages/ds-tokens/generated/composed.tokens.json` (`pnpm run tokens:build`) and friends inside the worktree before running gates. Scoped commits (`feat(…)`, `fix(…)`, `docs(…)`, one per logical phase). When done, run the full gate locally inside the worktree: `pnpm run governed:rail && pnpm test && pnpm run test:frameworks`. Plain `git checkout -b` branches bypass the binding/scope enforcement — this is a correctness requirement for concurrent agents, not a style preference.
...
src/                    # React showcase app (Vite) — <!-- src-top-level-dir-count -->12 top-level dirs
  ...
  router.tsx            # <!-- showcase-route-count -->13 hash routes (the Route union)
  views/                # <!-- src-view-count -->17 top-level view modules + sections/ evidence panels
...
- **Attribution is guarded — never add a `Co-authored-by:` trailer, and never author a commit as an agent.** GitHub turns one such trailer into a permanent, un-hideable repository contributor and charges a full history rewrite to undo it (measured 2026-09-23: 3,826 of main's 5,702 commit SHAs, across 14 branches). `.githooks/commit-msg` refuses the trailer at creation and any AI-agent author/committer identity; `.githooks/pre-push` and the CI `Attribution guard` step re-scan the pushed range for commits made with `--no-verify`. The policy lives in `scripts/attribution-guard.mjs` — `Co-authored-by` is **default-deny**, with an allowlist that is empty by design, so a vendor rotating its agent's address cannot slip through. The scan is deliberately range-scoped, never full-history: three grandfathered `Qwen-Coder` trailers from 2026-08-15 remain in `main` and cannot be removed without that rewrite. Check a range directly with `node scripts/attribution-guard.mjs --range '<base>..<tip>'`.
- **`CLAUDE.md` is gitignored** (`.gitignore`) and machine-local; the tracked agent-instruction surface is this file. Refresh your local copy when the guidance here changes.
