...
- React Native is a **rail-admitted default target**: it is in `admission-descriptor.ts`, `--target=all` and `governed:rail` emit and verify it, and CI's generated-tree diff covers `packages/ds-react-native/src`. It is not recon. SwiftUI and Jetpack Compose *are* explicit-only builtin targets outside rail verification and outside CI drift diffs — SwiftUI emits its allowlisted <!-- target-component-count:swiftui -->51 of the <!-- component-count -->52 corpus contracts and Compose emits its allowlisted <!-- target-component-count:jetpack-compose -->23, both compiled by dedicated CI native lanes over hand-authored example consumers, which proves compilation, not component correctness.
