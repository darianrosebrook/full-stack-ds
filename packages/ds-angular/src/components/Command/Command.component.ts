  ...
  <ng-container *ngIf="behavior.open()">
    <div [ngClass]="'command__dialog'" role="dialog" aria-modal="true" [attr.aria-label]="label">
      <div [ngClass]="'command__inputWrapper'">
        ...
        <input [ngClass]="'command__input'" type="search" role="combobox" aria-autocomplete="list" aria-controls="fsds-command-listbox" [attr.aria-expanded]="behavior.open()" [placeholder]="placeholder" [value]="behavior.search()" (change)="handleSearchChange($event)" />
      </div>
