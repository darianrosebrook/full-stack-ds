...
/**
 * Reset the shared override namespace at each component boundary. Defaults
 * live in consumer fallbacks, so this sheet cannot mask authored geometry.
 * Zero specificity lets a consumer target [data-fsds-component] directly.
 */
export function emitBoxModelBoundaryCss(shadowHost = false): string {
  const boundary = shadowHost ? ":where([data-fsds-component]), :host([data-fsds-component])" : ":where([data-fsds-component])";
  const slots = [...loadBoxModelPrimitive().slotNames];
  const axes = new Set(["padding-block", "padding-inline"]);
  const sides = new Set(["padding-block-start", "padding-block-end", "padding-inline-start", "padding-inline-end"]);
  const properties = slots.map(slot => slot.slice("box-model.".length));
  const controls = (layer: string, names: string[]) => `  @layer ${layer} {\n    :where([data-fsds-box]) {\n${names.map(name => `      ${name}: var(--fsds-box-model-${name}, revert-layer);`).join("\n")}\n    }\n  }`;
  return `/* @generated by ds-codegen from the box-model primitive */\n@layer components {\n  @layer primitive, defaults, box, axes, sides;\n  ${boundary} {\n${slots.map(slot => `    --fsds-${slot.replace(/\./g, "-")}: initial;`).join("\n")}\n  }\n${controls("box", properties.filter(p => !axes.has(p) && !sides.has(p)))}\n${controls("axes", [...axes])}\n${controls("sides", [...sides])}\n}\n`;
}
