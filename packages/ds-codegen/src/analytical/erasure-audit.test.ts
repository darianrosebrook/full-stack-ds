...
describe("the witness audit", () => {
  ...
  it("classifies all 63 witnesses and leaves none with an unresolved plan", () => {
    // 47 at stage 1.5, plus fourteen stage-2 necessities added by later adjudications.
    expect(live.witnesses.length).toBe(63);
  ...
  it("leaves the remaining forty-nine atomic, so the correction is bounded", () => {
    expect(byVerdict("atomic").length).toBe(49);
...
describe("the terminal invariant is measured over the population the report names", () => {
  ...
  it("admits each population description before comparing it, on either side", () => {
    ...
    expectProblem(run((r) => { r.scopes.sourceLanguageDeparture.specimens = 999; }), "RECORDED", "sourceLanguageDeparture scope covers 999 specimens, not the 124 authored", "source count 999");
...
describe("the specimen population shares ids, so a sweep must bind outcomes by CONTENT", () => {
  ...
  it("and an id-resolving sweep claims 167 bound specimens where content says 79", () => {
    const s = specimens();
    const oracle = loadOracle();
    expect(s.fixtures.filter((f) => hasOutcome(f, oracle)).length).toBe(167);
    ...
    expect(s.fixtures.filter((f) => hasOutcome(f, oracle) && !contentMatches(f, oracle)).length).toBe(88);
  });
