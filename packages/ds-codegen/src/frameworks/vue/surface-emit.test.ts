...
describe("vue surface-emit — anchored presence", () => {
  ...
  it("emits root, trigger, content SFCs and a composable for Tooltip", () => {
    const files = generateVueSurfaceFiles(irFor("Tooltip"));
    expect(files.rootSfc).toContain("tooltip");
    expect(files.triggerSfc).toContain("tooltip");
    expect(files.contentSfc).toContain("tooltip");
    expect(files.composable).toContain("open");
  });

  it("emits the part-key-lowered family for Popover", () => {
    const files = generateVueSurfaceFiles(irFor("Popover"));
    expect(files.rootSfc).toContain("popover");
    expect(files.triggerSfc).toContain("popover");
  });
});
