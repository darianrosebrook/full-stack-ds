...
describe("assertPreservableHasMarkers (FIX-CODEGEN-PRESERVABLE-MARKER-INVARIANT-01)", () => {
  it("throws a plain Error (not PreserveError) naming the file when a preservable output has no markers", () => {
    let caught: unknown;
    try {
      assertPreservableHasMarkers(
        "Switch/Switch.kt",
        "// @generated by ds-codegen — do not edit by hand.\npackage com.fullstackds.components.switch\n",
      );
    } catch (err) {
      caught = err;
    }
    // A file-header @generated comment alone is NOT a region marker.
    expect(caught).toBeInstanceOf(Error);
    expect(caught).not.toBeInstanceOf(PreserveError);
    expect((caught as Error).message).toContain("Switch/Switch.kt");
    expect((caught as Error).message).toContain("preservable");
    expect((caught as Error).message).toContain("no @generated markers");
  });

  it("passes marker-bearing output through untouched", () => {
    expect(() =>
      assertPreservableHasMarkers(
        "Switch/Switch.kt",
  ...
  it("accepts block-comment markers too", () => {
    expect(() =>
      assertPreservableHasMarkers(
        "Card/Card.css",
