...
<template>
  <input :class="classNames" @input="(e) => behavior.setValue((e.target as HTMLInputElement).value)" :value="behavior.value.value" :disabled="props.disabled" :aria-invalid="props.invalid" :type="props.type" :placeholder="props.placeholder" :name="props.name" :required="props.required" :aria-label="props.ariaLabel" :aria-labelledby="props.ariaLabelledby" role="textbox" :data-testid="props['data-testid']" data-fsds-component="input" :id="fieldAssociation?.controlId" :aria-describedby="fieldAssociation?.describedBy" />
</template>
