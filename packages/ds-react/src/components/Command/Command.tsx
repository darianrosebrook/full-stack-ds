      ...
      {open ? (
        <div className="command__dialog" role="dialog" aria-modal="true" aria-label={label}>
          <div className="command__inputWrapper">
            ...
            <input className="command__input" type="search" role="combobox" aria-autocomplete="list" onInput={(e) => setSearch((e.currentTarget as HTMLInputElement).value)} aria-expanded={open} aria-label={searchLabel} placeholder={placeholder} value={search} id={`${instanceId}-input`} aria-controls={`${instanceId}-list`} />
          </div>
