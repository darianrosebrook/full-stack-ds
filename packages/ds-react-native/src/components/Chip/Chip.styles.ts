...
export function createChipStyles(theme?: FsdsTheme) {
  ...
  return StyleSheet.create({
    ...
    rootText: { color: (tokens.root?.["chip.color.foreground.default"] as string | undefined), fontSize: (tokens.root?.["chip.text.size"] as number | undefined), fontWeight: (tokens.root?.["chip.text.weight"] as TextStyle["fontWeight"]) },
    text: {},
  });
}
