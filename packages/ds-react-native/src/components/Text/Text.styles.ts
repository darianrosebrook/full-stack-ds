...
export function createTextStyles(theme?: FsdsTheme) {
  ...
  return StyleSheet.create({
    ...
    root_variant_body: definedStyle({ fontWeight: (tokens.root?.["text.typography.fontWeight.regular"] as TextStyle["fontWeight"]), color: "#555555" }),
    root_variant_title: definedStyle({ fontWeight: (tokens.root?.["text.typography.fontWeight.bold"] as TextStyle["fontWeight"]), color: (tokens.root?.["text.color.foreground.primary"] as string | undefined) }),
  });
}
