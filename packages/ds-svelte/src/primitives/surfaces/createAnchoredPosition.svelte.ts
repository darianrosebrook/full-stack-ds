...
export interface CreateAnchoredPositionOptions {
  ...
  /** Getter for gap in pixels between anchor and content. Defaults to 8. */
  offset?: () => number;
  /**
   * Getter for minimum gap to viewport edges when clamping horizontally /
   * vertically via "shift". Defaults to 8.
   */
...
export interface CreateAnchoredPositionResult {
  /** Reactive snapshot of the current position state (getter). Callers
   *  read this in `$derived(...)` or directly in the template. */
  readonly state: AnchoredPositionState;
}
 ...
 *
 * Designed to compose with `createAnchoredSurface`: pass the anchor and
...
export function createAnchoredPosition(
  ...
  const getPlacement = options.placement ?? (() => "auto" as const);
  const getCollision = options.collision ?? (() => "flip-shift" as const);
  const getOffset = options.offset ?? (() => 8);
  const getViewportPadding = options.viewportPadding ?? (() => 8);

  let state = $state<AnchoredPositionState>(HIDDEN_STATE);

  function compute(): void {
    const anchor = options.anchor();
    const content = options.content();
    if (!options.open() || !anchor || !content) return;
    const next = computePosition({
      ...
      offset: getOffset(),
      viewportPadding: getViewportPadding(),
    });
    // Avoid spurious writes when the computed values match.
    if (
      state.ready &&
  ...
  $effect(() => {
    ...
    // Re-read placement/collision/offset/viewportPadding here so the
    // effect re-runs when any of them change while open.
    getPlacement();
    getCollision();
    getOffset();
    getViewportPadding();
...
export function computePosition(args: ComputeArgs): AnchoredPositionState {
