...
describe("Divider — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-divider");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("divider");
  });

  it("applies orientation=horizontal variant class", async () => {
    const { element } = await renderElement("fsds-divider", { "orientation": "horizontal" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("divider--horizontal");
  });

  it("applies orientation=vertical variant class", async () => {
    const { element } = await renderElement("fsds-divider", { "orientation": "vertical" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("divider--vertical");
  });
});
