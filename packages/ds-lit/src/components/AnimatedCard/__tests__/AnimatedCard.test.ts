...
describe("AnimatedCard — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-animated-card");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("animated-card");
  });

  it("applies as=article variant class", async () => {
    const { element } = await renderElement("fsds-animated-card", { "as": "article" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("animated-card--article");
  });

  it("applies as=div variant class", async () => {
    const { element } = await renderElement("fsds-animated-card", { "as": "div" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("animated-card--div");
  });

  it("applies as=li variant class", async () => {
    const { element } = await renderElement("fsds-animated-card", { "as": "li" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("animated-card--li");
  });

  it("applies as=a variant class", async () => {
    const { element } = await renderElement("fsds-animated-card", { "as": "a" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("animated-card--a");
  });
});
