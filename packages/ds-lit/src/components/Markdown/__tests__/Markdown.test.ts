...
const requiredProps = { "content": "placeholder" };

describe("Markdown — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-markdown");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("markdown");
  });
});

describe("Markdown — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-markdown", { "content": "placeholder" });
