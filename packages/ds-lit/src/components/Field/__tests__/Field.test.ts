...
const requiredProps = { "name": "placeholder" };

describe("Field — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-field");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("field");
  });

  it("applies status=idle variant class", async () => {
    const { element } = await renderElement("fsds-field", { "status": "idle" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("field--idle");
  });

  it("applies status=validating variant class", async () => {
    const { element } = await renderElement("fsds-field", { "status": "validating" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("field--validating");
  });

  it("applies status=valid variant class", async () => {
    const { element } = await renderElement("fsds-field", { "status": "valid" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("field--valid");
  });

  it("applies status=invalid variant class", async () => {
    const { element } = await renderElement("fsds-field", { "status": "invalid" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("field--invalid");
  });
});

describe("Field — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-field", { "name": "placeholder" }, [{"slotName":"label","html":"<span>Test Field label</span>"}]);
