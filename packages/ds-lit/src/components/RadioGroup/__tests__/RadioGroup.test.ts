...
describe("RadioGroup — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-radio-group");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("radio-group");
  });

  it("applies orientation=vertical variant class", async () => {
    const { element } = await renderElement("fsds-radio-group", { "orientation": "vertical" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("radio-group--vertical");
  });

  it("applies orientation=horizontal variant class", async () => {
    const { element } = await renderElement("fsds-radio-group", { "orientation": "horizontal" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("radio-group--horizontal");
  });
});

describe("RadioGroup — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-radio-group", { "name": "placeholder", "ariaLabel": "Test RadioGroup" });
