...
describe("NavList — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-nav-list");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("nav-list");
  });

  it("applies orientation=vertical variant class", async () => {
    const { element } = await renderElement("fsds-nav-list", { "orientation": "vertical" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("nav-list--vertical");
  });

  it("applies orientation=horizontal variant class", async () => {
    const { element } = await renderElement("fsds-nav-list", { "orientation": "horizontal" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("nav-list--horizontal");
  });
});

describe("NavList — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-nav-list", { "ariaLabel": "Test NavList" }, [{"html":"<li>content</li>"}]);
