...
describe("OTP — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-otp");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("otp");
  });

  it("applies mode=numeric variant class", async () => {
    const { element } = await renderElement("fsds-otp", { "mode": "numeric" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("otp--numeric");
  });

  it("applies mode=alphanumeric variant class", async () => {
    const { element } = await renderElement("fsds-otp", { "mode": "alphanumeric" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("otp--alphanumeric");
  });
});

describe("OTP — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-otp", { "label": "Test OTP" });
