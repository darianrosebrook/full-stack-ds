...
describe("Tabs — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-tabs");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("tabs");
  });

  it("applies orientation=horizontal variant class", async () => {
    const { element } = await renderElement("fsds-tabs", { "orientation": "horizontal" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tabs--horizontal");
  });

  it("applies orientation=vertical variant class", async () => {
    const { element } = await renderElement("fsds-tabs", { "orientation": "vertical" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tabs--vertical");
  });

  it("applies activationMode=automatic variant class", async () => {
    const { element } = await renderElement("fsds-tabs", { "activationMode": "automatic" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tabs--automatic");
  });

  it("applies activationMode=manual variant class", async () => {
    const { element } = await renderElement("fsds-tabs", { "activationMode": "manual" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tabs--manual");
  });
});
