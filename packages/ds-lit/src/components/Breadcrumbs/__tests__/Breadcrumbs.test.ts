...
describe("Breadcrumbs — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-breadcrumbs");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("breadcrumbs");
  });
});

describe("Breadcrumbs — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-breadcrumbs", { "ariaLabel": "Test Breadcrumbs" }, [{"html":"<li>content</li>"}]);
