...
export class SelectElement extends LitElement {
  static override styles = css`
    ...
    .select__text {
      ...
      white-space: nowrap;
      text-align: var(--fsds-select-design-text-typography-alignment, left);
    }
    ...
    .select__options {
      display: flex;
      flex-direction: column;
      gap: var(--fsds-select-design-options-spacing-gap, 2px);
      list-style: none;
      padding: var(--fsds-select-design-options-spacing-padding, 0);
      margin: 0;
    }
    ...
    .select__trigger:focus-visible:not([aria-disabled="true"]) {
      outline-width: var(--fsds-select-design-condition-c1f07e6a65af-focus-width, var(--fsds-select-focus-ring-width, 2px));
      outline-color: var(--fsds-select-design-condition-c1f07e6a65af-focus-color, var(--fsds-select-focus-ring-color, #0566fe));
      outline-style: var(--fsds-select-design-condition-c1f07e6a65af-focus-style, var(--fsds-select-focus-ring-style, solid));
      outline-offset: var(--fsds-select-design-condition-c1f07e6a65af-focus-offset, var(--fsds-select-focus-ring-offset, 2px));
    }

    .select--disabled .select__trigger {
      cursor: not-allowed;
      opacity: var(--fsds-select-design-condition-98ff1c58463e-appearance-opacity, 0.5);
    }

    .select--sm .select__trigger {
      height: var(--fsds-select-design-condition-37dc4084a16a-sizing-height, var(--fsds-select-size-sm-height, 24px));
    }

    .select--md .select__trigger {
      height: var(--fsds-select-design-condition-33ea5912860e-sizing-height, var(--fsds-select-size-md-height, 32px));
    }
  ...
  private get behavior(): SelectBehavior {
    ...
    get anchorEl() { return host.interactionAnchor; },
    value: () => this.value,
  ...
  override render() {
    return html`<div class="${this.computeClasses()}" role="combobox" aria-haspopup="listbox" aria-label=${ifDefined((this.triggerLabel ?? "Select an option"))} aria-expanded=${this.behavior.open ? 'true' : 'false'} aria-disabled=${ifDefined(this.disabled === undefined ? undefined : (this.disabled ? 'true' : 'false'))} aria-controls=${ifDefined([this.open ? 'select-content' : null].filter(Boolean).join(' ') || undefined)} data-fsds-box="">
  <button class=${'select__trigger'} ${ref(element => { this.interactionAnchor = element instanceof HTMLElement ? element : undefined; })} type="button" @click=${() => this.behavior.setOpen(!this.behavior.open)} @keydown=${(e: KeyboardEvent) => this.behavior.handleTriggerKeydown(e)} ?disabled=${this.disabled ?? false} aria-label=${ifDefined((this.triggerLabel ?? "Select an option"))} aria-expanded=${this.behavior.open ? 'true' : 'false'} aria-controls=${ifDefined([this.open ? 'select-content' : null].filter(Boolean).join(' ') || undefined)}>
    <span class=${'select__text'}>${(((this.options ?? [{"value":"alpha","label":"Alpha"},{"value":"beta","label":"Beta"},{"value":"gamma","label":"Gamma"}]) || []).filter(option => (Array.isArray(this.behavior.selection) ? this.behavior.selection : [this.behavior.selection]).includes(option.value)).map(option => option.label).join(', ') || (this.placeholder ?? "Select an option"))}</span>
  </button>
