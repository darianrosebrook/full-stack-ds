...
describe("Tooltip — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-tooltip");
    expect(element).toBeInstanceOf(HTMLElement);
  });

  it("applies the base CSS class", async () => {
    const { element } = await renderElement("fsds-tooltip", { "open": true });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tooltip");
  });

  it("applies placement=top variant class", async () => {
    const { element } = await renderElement("fsds-tooltip", { "open": true, "placement": "top" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tooltip--top");
  });

  it("applies placement=bottom variant class", async () => {
    const { element } = await renderElement("fsds-tooltip", { "open": true, "placement": "bottom" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tooltip--bottom");
  });

  it("applies placement=left variant class", async () => {
    const { element } = await renderElement("fsds-tooltip", { "open": true, "placement": "left" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tooltip--left");
  });

  it("applies placement=right variant class", async () => {
    const { element } = await renderElement("fsds-tooltip", { "open": true, "placement": "right" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tooltip--right");
  });

  it("applies placement=auto variant class", async () => {
    const { element } = await renderElement("fsds-tooltip", { "open": true, "placement": "auto" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("tooltip--auto");
  });
