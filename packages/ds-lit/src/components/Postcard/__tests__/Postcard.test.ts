...
const requiredProps = { "postId": "placeholder", "author": {}, "timestamp": "placeholder", "stats": {} };

describe("Postcard — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-postcard");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("postcard");
  });
});

describe("Postcard — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-postcard", { "postId": "placeholder", "author": {}, "timestamp": "placeholder", "stats": {}, "aria-label": "Test Postcard" }, [{"html":"<span>content</span>"}]);
