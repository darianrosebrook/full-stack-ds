...
describe("Walkthrough — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-walkthrough");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("walkthrough");
  });

  it("applies placement=top variant class", async () => {
    const { element } = await renderElement("fsds-walkthrough", { "placement": "top" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("walkthrough--top");
  });

  it("applies placement=bottom variant class", async () => {
    const { element } = await renderElement("fsds-walkthrough", { "placement": "bottom" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("walkthrough--bottom");
  });

  it("applies placement=left variant class", async () => {
    const { element } = await renderElement("fsds-walkthrough", { "placement": "left" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("walkthrough--left");
  });

  it("applies placement=right variant class", async () => {
    const { element } = await renderElement("fsds-walkthrough", { "placement": "right" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("walkthrough--right");
  });

  it("applies placement=auto variant class", async () => {
    const { element } = await renderElement("fsds-walkthrough", { "placement": "auto" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("walkthrough--auto");
  });
});

describe("Walkthrough — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-walkthrough", {}, [{"slotName":"title","html":"<span>Test Walkthrough title</span>"},{"slotName":"description","html":"<span>Test Walkthrough description</span>"}]);
