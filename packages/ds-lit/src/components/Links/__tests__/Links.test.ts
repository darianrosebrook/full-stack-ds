...
describe("Links — unit", () => {
  it("renders with default props", async () => {
    const { element } = await renderElement("fsds-links");
  ...
  it("applies the base CSS class", async () => {
    ...
    expect(classTokens(root)).toContain("links");
  });

  it("applies size=small variant class", async () => {
    const { element } = await renderElement("fsds-links", { "size": "small" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("links--small");
  });

  it("applies size=medium variant class", async () => {
    const { element } = await renderElement("fsds-links", { "size": "medium" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("links--medium");
  });

  it("applies size=large variant class", async () => {
    const { element } = await renderElement("fsds-links", { "size": "large" });
    const root = element.shadowRoot?.firstElementChild ?? element;
    expect(classTokens(root)).toContain("links--large");
  });
});

describe("Links — accessibility", () => {
  it("has no unexpected axe violations with default props", async () => {
    const { element } = await renderElement("fsds-links", {}, [{"html":"<span>content</span>"}]);
