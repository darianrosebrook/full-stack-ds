...
packages/
  ...
  ds-codegen/                    # TypeScript codegen CLI + IR + emitters + rail
    src/
      ...
      frameworks/
        ...
        swift/  jetpack-compose/  # Explicit-only targets, outside the rail
  ...
  ds-swiftui/  ds-jetpack-compose/ # Generated native packages, outside the rail
...
| Flag | Purpose |
|---|---|
| `--target=<list>` | Comma-separated target ids: `react`, `vue`, `svelte`, `angular`, `lit`, `react-native`, `figma`, or `all`. `swiftui` and `jetpack-compose` are explicit-only — requestable by name, but never included in `--target=all`. |
...
CI runs `governed:rail` and then refuses generated drift with `git diff --exit-code` over the six framework `src` trees (the five Web DOM packages plus `packages/ds-react-native/src`). The order is load-bearing: regenerate/admit first, then prove the regenerated bytes were already committed.
...
`pnpm run e2e:rail` runs the Playwright runtime fact rail in `e2e/runtime-rail.spec.ts`. It exercises selected contract facts for Progress, Truncate, ShowMore, OTP, and Calendar through the <!-- web-framework-count -->5 web-framework preview mounts.

The rail proves narrow runtime facts across **all five** Web DOM frameworks including Angular: default *and* non-default CSS-var fallback behavior, and iteration DOM shape, for the selected components. It does not prove cross-framework behavioral parity beyond DOM shape, or visual quality. The one Angular exclusion is the Select callback-capture interaction test, skipped because `angular-preview` uses a synthesized-host pipeline rather than the shared `config-entry.ts` callback bus — not because Angular preview is unproven. Screenshot baselines are local darwin baselines and skip under `CI=true`; CI runs the OS-agnostic fact assertions.
