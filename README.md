...
The current curated catalog killed all five sampled mutants at `3022618b`: two structurally, two through mixed authored-test evidence, and one through a contract-derived audit. That is evidence about those five mutants, not a general mutation score or proof that every contract field has an oracle. The report's decisive-evidence class is the useful output: surviving field classes, as the catalog grows, identify where new authored or external oracles pay.
